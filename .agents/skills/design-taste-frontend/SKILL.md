...
### 4.5.A Async Loading States

For an asynchronous surface with more than one small control, render a skeleton instead of a spinner during its initial load.

- **Match the final geometry:** Skeleton blocks must reserve the same card, row, chart, or form layout that will appear after loading. Preserve the expected height and responsive grid so the content does not jump.
- **Use the project primitive:** Prefer the existing `Skeleton` component and semantic design tokens. Do not hand-roll `animate-pulse` placeholders or use arbitrary gray colors.
- **Initial load vs. refetch:** Show skeletons only when the surface has no usable data yet. During a background refetch, keep the current content visible and indicate pending work locally if needed. Do not flash back to a skeleton.
- **Route-level loading:** For asynchronous route segments, add a `loading.tsx` that mirrors the page shell and its largest content block. Do not replace an entire page with a centered spinner.
- **Scope the placeholder:** A single button action may use an inline spinner and disabled state. Tables, cards, charts, forms, and content sections need layout-matched skeletons.
- **Keep states distinct:** Loading, empty, and error states each need their own branch. A skeleton must not be used as a permanent fallback or to hide a failed request.
...
- [ ] **Async loading geometry** matches the final component layout, prevents layout shift, and does not replace existing data during background refetch?
